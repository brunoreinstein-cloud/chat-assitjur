...
import { historyFetcher } from "@/lib/utils";
...
function removeChatFromHistories(
  chatHistories: ChatHistory[] | undefined,
  chatId: string
  ...
  return chatHistories.map((chatHistory) => ({
    ...chatHistory,
    chats: chatHistory.chats.filter((c) => c.id !== chatId),
  }));
}
...
export function SidebarHistory({
  ...
  const allChats = paginatedChatHistories
    ...
    : [];
  const filteredChats = filterChatsByQuery(allChats, searchQuery);
  const hasEmptyChatHistory = allChats.length === 0;
  const hasNoSearchResults =
    searchQuery.length > 0 && filteredChats.length === 0 && !isLoading;

  const applyDeleteToCache = (chatId: string) => {
    mutate((chatHistories) => removeChatFromHistories(chatHistories, chatId));
  };
  ...
  const handleDelete = () => {
    ...
    toast.promise(deletePromise, {
      ...
      success: (result) => {
        ...
        applyDeleteToCache(chatToDelete);
        if (isCurrentChat) {
          router.replace("/");
          router.refresh();
        }
      ...
      <AlertDialog onOpenChange={setShowDeleteDialog} open={showDeleteDialog}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Esta ação não pode ser desfeita. O chat será eliminado
              permanentemente e removido dos nossos servidores.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
