...
import { AgentInstructionsDialog } from "./multimodal-input/agent-instructions-dialog";
import { AttachmentChips } from "./multimodal-input/attachment-chips";
import { AttachmentsButton } from "./multimodal-input/attachments-button";
import { DragDropOverlay } from "./multimodal-input/drag-drop-overlay";
import { ManageAgentsSheet } from "./multimodal-input/manage-agents-sheet";
import { RevisorHint } from "./multimodal-input/revisor-hint";
import { RevisorStatusBanner } from "./multimodal-input/revisor-status-banner";
import { StopButton } from "./multimodal-input/stop-button";
import { useCustomAgents } from "./multimodal-input/use-custom-agents";
...
function PureMultimodalInput({
  ...
  } = useCustomAgents({ agentId, setAgentId, enabled: !!setAgentId });
      ...
      >
        ...
        {messages.length === 0 && agentId === "revisor-defesas" && (
          <RevisorHint
            ...
            pendingPreferredTypeRef={pendingPreferredTypeRef}
          />
        )}
        {(attachments.length > 0 || uploadQueue.length > 0) && (
          <AttachmentChips
            ...
            onExpandChips={() => setChipsExpanded(true)}
            onRemoveAttachment={handleRemoveAttachment}
            uploadQueue={uploadQueue}
          />
        )}
        {isRevisorAgent && (messages.length === 0 || hasPiOrContestacao) && (
          <RevisorStatusBanner
            fileInputRef={fileInputRef}
            hasContestacao={hasContestacao}
            hasPi={hasPi}
            hasPiAndContestacao={hasPiAndContestacao}
            pendingPreferredTypeRef={pendingPreferredTypeRef}
          />
        )}
            ...
            {setAgentId && (
              <>
                <ManageAgentsSheet
                  ...
                  onRefreshKnowledge={mutateKnowledgeForAgentForm}
                  onSave={saveAgentForm}
                  onStartCreate={startCreateAgent}
                ...
                {setAgentInstructions && (
                  <AgentInstructionsDialog
                    isImproving={isImproving}
                    localInstructions={localAgentInstructions}
                    onImproveInstructions={handleImproveInstructions}
                    onOpenChange={handleAgentInstructionsDialogOpenChange}
                    open={agentInstructionsDialogOpen}
                    setLocalInstructions={setLocalAgentInstructions}
                  />
                )}
              </>
            )}
