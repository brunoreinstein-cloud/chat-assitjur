...
const TOOL_STATE_ICONS: Record<string, ReactNode> = {
...
const getStatusBadge = (status: ToolUIPart["state"] | undefined) => {
  if (status == null) {
    return (
      <Badge
  ...
  const label = TOOL_STATE_LABELS[status] ?? "—";
  const icon = TOOL_STATE_ICONS[status] ?? <CircleIcon className="size-4" />;
