...
| 4   | **Quick wins SPEC (análise BR Consultoria)** | ~~(a) `searchJurisprudencia`~~ ✅ ~~(c) IP Lock~~ ✅ (verificado: mensagem uniforme em todos os 5 agentes). Pendentes: (b) `/ajuda` no chat (~2h); (d) Checklist pré-entrega no DOCX (~2h). Ver §11.6 e §12. | — |
