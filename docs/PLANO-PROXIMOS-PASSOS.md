...
| 0c  | **🔴 RLS completo no Supabase** | ~~Migração `0037_rls_policies.sql` criada~~: habilita RLS em todas as 23 tabelas sensíveis + política `service_role_all` para role postgres. Aplicar em produção: `pnpm run db:migrate`. Pendente: teste automatizado que valida RLS. | 🟡 parcial |
| 1   | **Executar migrações manuais em produção** | `pnpm run db:migrate` (com `POSTGRES_URL` de produção) — aplica `0034_peca_status.sql`, `0035_processo_fase_processual.sql`, `0036_credit_transactions.sql`, `0037_rls_policies.sql`. | — |
...
| 4   | **Quick wins SPEC (análise BR Consultoria)** | ~~(a) `searchJurisprudencia`~~ ✅ ~~(b) `/ajuda` em todos os agentes~~ ✅ ~~(c) IP Lock~~ ✅ ~~(d) Checklist pré-entrega na Seção 7 do DOCX~~ ✅. Todos concluídos. Ver §11.6 e §12. | ✅ |
