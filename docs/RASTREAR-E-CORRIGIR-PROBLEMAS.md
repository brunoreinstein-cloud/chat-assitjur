...
| **persistChat** | Verificar ownership, criar chat novo (se id vazio), gerar título (`saveChat`) | Entre dbBatch e validationRag; se lento, atrasa o arranque do stream. DEBUG_CHAT: ver ordem em `preStreamPhases`. |
...
4. **Identificar a fase** com maior tempo (dbBatch, persistChat, validationRag, saveMessages, etc.).
